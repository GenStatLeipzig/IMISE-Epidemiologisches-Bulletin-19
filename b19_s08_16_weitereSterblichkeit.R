...
calcNEutod <- function(
  erhoehung_impf_ue60=0, 
  showAllHist = F, 
  zusatzschutzfaktorImpfung3_min = 1,
  zusatzschutzfaktorImpfung3_max = 1, 
  impfschutz_80plus_beta = T,
  impfschutz_80plus_min = 0.55, 
  impfschutz_80plus_max = 0.85,   
  impfschutz_60_80_min = 0.80, 
  impfschutz_60_80_max = 0.90,  
  betroffen_ungeimpft_min = 0.3, 
  betroffen_ungeimpft_max = 0.6, 
  betroffen_geimpft_infiziert_min = 0.3, 
  betroffen_geimpft_infiziert_max = 0.6
) 
{
  ...
  # simulate fatality rate distribution for unvacc pop age >80
  # basierenden auf RKI github
  ...
  # bis 15.1.21, i.e. vor Impfung
  cfr_80plus_ungeimpft_min = 0.16 
  ...
  # simulate fatality rate distribution for unvacc pop age 60-80
  ...
  # simulate distribution of vaccination protection based on specified
  # distribution for age group >80 (beta or uniform distribution)
  if(impfschutz_80plus_beta ==T) {
    ...
    impfschutz_80plus = runif(n = Nsim, 
                              min = impfschutz_80plus_min, 
                              max = impfschutz_80plus_max) 
  ...
  # simulate case fatalities for pop age >80 vacc without booster
  cfr_80plus_geimpft_ohneBooster = cfr_80plus_ungeimpft * (1-impfschutz_80plus)
  
  # simulate protection by booster (third) vaccination) from uniform distribution
  zusatzschutzfaktorImpfung3 = runif(
    Nsim,
    zusatzschutzfaktorImpfung3_min,
    zusatzschutzfaktorImpfung3_max
  )
  
  # simulate fatalities for triple vacc. pop >80
  ...
  # simulate protection rate of vaccination in pop age 60-80
  impfschutz_60_80 = runif(
    n = Nsim, 
    min = impfschutz_60_80_min, 
    max = impfschutz_60_80_max
  ) 
  
  if(showAllHist==T) NiceHist(impfschutz_60_80)
  range(impfschutz_60_80)
  
  # simulate fatalities for pop age 60-80 with and without booster vaccination
  ...
  betroffen_ungeimpft = rtriangle(
    n = Nsim, 
    a = betroffen_ungeimpft_min, 
    b = betroffen_ungeimpft_max)
  
  if(showAllHist==T) NiceHist(betroffen_ungeimpft)
  
  # simulate infection rate for vacc pop
  betroffen_geimpft_infiziert = rtriangle(
    n = Nsim, 
    a = betroffen_geimpft_infiziert_min, 
    b = betroffen_geimpft_infiziert_max
  )
  
  if(showAllHist==T) NiceHist(betroffen_geimpft_infiziert)
  
  # total pop count saxony
  # Einwohner Sachsen nach Alter
  # https://www-genesis.destatis.de/genesis/online?operation=previous&levelindex=2&step=2&titel=Ergebnis&levelid=1634026654759&acceptscookies=false#abreadcrumb
  N60_80 = 1011120
  N80plus = 361987  
  
  # vacc rate for age pops
  # Impfquote von https://www.rki.de/DE/Content/InfAZ/N/Neuartiges_Coronavirus/Daten/Impfquoten-Tab.html 
  impfquote_60_80_min = 0.778  + erhoehung_impf_ue60 
  
  # potential max vacc rate in age group 60-80 (because of underreporting etc.)
  # COVIMO  meint, dass darueber hinaus die Impfquoten generell wegen
  # Untererfassung hoeher sind
  ...
  impfquote_60_80_max = impfquote_60_80_min + 0.05
  
  # 
  impfquote_60_80_pre = rtriangle(
    n = Nsim, 
    a = impfquote_60_80_min, 
    b = impfquote_60_80_max, 
    c = impfquote_60_80_max
  )
  
  if(showAllHist==T) NiceHist(impfquote_60_80_pre)
  
  # simulate increase of vacc rate in age pop >80
  # in IPSOS Sachsen und Vgl.
  # https://www.rki.de/DE/Content/InfAZ/N/Neuartiges_Coronavirus/Situationsberichte/Jun_2021/2021-06-02-de.pdf?__blob=publicationFile
  # Sonderauswertung Impfquoten Impfquote nicht gleich in ue60, sondern in ue80
  # hoeher
  erhoehung_impf_80 = runif(
    n = Nsim, 
    min = 0.02, 
    max = 0.08
  )
  
  #  minuskorrektur_ungleichverteilungImpfung_60_80  wegen Annahme Impfquote bei
  #  80plus Jaehrigen ca 5-10% hoeher als 60-80, muss dann ja bei den juengeren
  #  ausgeglichen werden.
  korrektur_ungleichverteilungImpfung_60_80 = erhoehung_impf_80*N80plus/N60_80
  summary(korrektur_ungleichverteilungImpfung_60_80)
  
  # correct vacc rate in pop age 60-80
  impfquote_60_80 = impfquote_60_80_pre - korrektur_ungleichverteilungImpfung_60_80
  
  # no vacc rate >1 possible -> correct all values to 1
  ...
  # vacc rate pop age >80
  # https://www.rki.de/DE/Content/InfAZ/N/Neuartiges_Coronavirus/Daten/Impfquoten-Tab.html s.o. generell hoeher in aelteren  
  impfquote_80plus_min = 0.778  + erhoehung_impf_ue60
  
  # potential vacc rate (probably higher than reported)
  # COVIMO  berichtet von wahrscheinlich hoeherer Impfquote https://www.rki.de/DE/Content/InfAZ/N/Neuartiges_Coronavirus/Projekte_RKI/COVIMO_Reports/covimo_studie_bericht_7.pdf?__blob=publicationFile 
  impfquote_80plus_max =  impfquote_80plus_min + 0.05
  
  # calc vaccination rate
  impfquote_80plus_pre = rtriangle(
    n = Nsim, 
    a = impfquote_80plus_min, 
    b = impfquote_80plus_max, 
    c = impfquote_80plus_max
  )
  if(showAllHist==T) NiceHist(impfquote_80plus_pre)
  
  # adjust vacc rate probability and remove probabilities >1
  ...
  # simulate overlap of vacc and infected pop
  ...
  ueberlapp_infiziertUNDgeimpft_60_80 = rtriangle(
    n = Nsim, 
    a = ueberlapp_infiziertUNDgeimpft_60_80_min, 
    b = ueberlapp_infiziertUNDgeimpft_60_80_max, 
    c=mean(impfquote_60_80)
  ) # 0.8 ca. saechsische impfquote 60+
  ...
  ueberlapp_infiziertUNDgeimpft_80plus = rtriangle(
    n = Nsim, 
    a = ueberlapp_infiziertUNDgeimpft_80plus_min, 
    b = ueberlapp_infiziertUNDgeimpft_80plus_max, 
    c=mean(impfquote_80plus)
  ) # 0.8 ca. saechsische impfquote 60+
  ...
  # deceased count in two pops
  verstorben_60_80  = 2936  # 12.10.21  
  verstorben_80plus  = 6993 # 12.10.21  
  
  # tested infections count in two pops
  ...
  # die geschuetzten, i.e. geimpfte plus genesene abzueglich genesene, die schon
  # bei den geimpften mit gezaehlt wurden
  infiziert_ungeimpft_60_80 = infiziert_lebend_60_80 - infiziert_lebend_60_80*ueberlapp_infiziertUNDgeimpft_60_80
  
  infiziert_ungeimpft_60_80 = ifelse(
    infiziert_ungeimpft_60_80 > (N60_80 - verstorben_60_80),
    (N60_80 - verstorben_60_80), 
    infiziert_ungeimpft_60_80 ) # es koennen nicht mehr infiziert sein, als da sind                                                              
  ...
  if(showAllHist==T){
    ...
    par(mfrow = c(3,1))
    NiceHist(geimpft_60_80)
    NiceHist(infiziert_ungeimpft_60_80)
    NiceHist(ungeschuetzt_60_80)
  ...
  # die geschuetzten, i.e. geimpfte plus genesene abzueglich genesene, die schon
  # bei den geimpften mit gezaehlt wurden
  infiziert_ungeimpft_80plus = infiziert_lebend_80plus - infiziert_lebend_80plus*ueberlapp_infiziertUNDgeimpft_80plus 
  infiziert_ungeimpft_80plus = ifelse(
    infiziert_ungeimpft_80plus > (N80plus - verstorben_80plus),
    (N80plus - verstorben_80plus),
    infiziert_ungeimpft_80plus) # es koennen nicht mehr infiziert sein, als da sind                                                              
  ...
  if(showAllHist==T){
    ...
    par(mfrow = c(3,1))
    NiceHist(geimpft_80plus)
    NiceHist(infiziert_ungeimpft_80plus)
    NiceHist(ungeschuetzt_80plus)
  ...
  if(showAllHist==T) NiceHist(neutod_ue60)
  quantile = quantile(neutod_ue60, probs = c(0.025, 0.5, 0.975))
  
  quantile_schutzdet = data.table(
    gruppe = factor(c("neutod_ue60", 
                      "neutod_ue60_ungeschuetzt",
                      "neutod_geimpft_infiziert" ),
                    levels = c("neutod_ue60", 
                               "neutod_ue60_ungeschuetzt",
                               "neutod_geimpft_infiziert" ), 
                    labels = c("Gesamt",
                               "Ungeschützt",
    ...
    rbind(
      c(
        min = min(neutod_ue60), 
        quantile(neutod_ue60, probs = c(0.025, 0.5, 0.975)), 
        max = max(neutod_ue60)
      ),
      c(
        min = min(neutod_ue60_ungeschuetzt),
        quantile(neutod_ue60_ungeschuetzt, probs = c(0.025, 0.5, 0.975)), 
        max = max(neutod_ue60_ungeschuetzt)
      ),
      c(
        min = min(neutod_geimpft_infiziert), 
        quantile(neutod_geimpft_infiziert, probs = c(0.025, 0.5, 0.975)),
        max = max(neutod_geimpft_infiziert))
    ) %>% round()
  )
  
  quantile_ageANDschutzdet = data.table(
    gruppe = factor(x = c("neutod_ue60", 
                          "neutod_ungeschuezt_60_80",
                          "neutod_ungeschuezt_80plus", 
                          "neutod_geimpft_infiziert_60_80",
                          "neutod_geimpft_infiziert_80plus" ),
                    levels =c("neutod_ue60", 
                              "neutod_ungeschuezt_60_80",
                              "neutod_ungeschuezt_80plus",
                              "neutod_geimpft_infiziert_60_80",
    ...
    rbind(
      ...
      c(
        min = min(neutod_ungeschuezt_60_80),
        quantile(neutod_ungeschuezt_60_80, probs = c(0.025, 0.5, 0.975)), 
        max = max(neutod_ungeschuezt_60_80)
      ),
      c(
        min = min(neutod_ungeschuezt_80plus), 
        quantile(neutod_ungeschuezt_80plus, probs = c(0.025, 0.5, 0.975)), 
        max = max(neutod_ungeschuezt_80plus)
      ),
      c(
        min = min(neutod_geimpft_infiziert_60_80), 
        quantile(neutod_geimpft_infiziert_60_80, probs = c(0.025, 0.5, 0.975)),
        max = max(neutod_geimpft_infiziert_60_80)
      ),
      c(
        min = min(neutod_geimpft_infiziert_80plus), 
        quantile(neutod_geimpft_infiziert_80plus, probs = c(0.025, 0.5, 0.975)), 
        max = max(neutod_geimpft_infiziert_80plus)
      )
    ) %>% round())
  
  quantile_agedet = data.table(
    gruppe = factor(x = c("neutod_ue60", "neutod_60_80","neutod_80plus"),
    ...
    rbind(
      ...
      c(
        min = min(neutod_60_80),
        quantile(neutod_60_80, probs = c(0.025, 0.5, 0.975)), 
        max = max(neutod_60_80)
      ),
      c(
        min = min(neutod_80plus),
        quantile(neutod_80plus, probs = c(0.025, 0.5, 0.975)),
        max = max(neutod_80plus)
      )
    ) %>% round())
  ...
  tabelle = data.table(
    ...
    ungeschuetzt_80plus,
    betroffen_ungeimpft,
    cfr_80plus_ungeimpft,
    dunkelziffer_60_80,
    dunkelziffer_80plus,
    N80plus ,
    verstorben_80plus ,
    geimpft_80plus,
    infiziert_ungeimpft_80plus,
    impfquote_80plus,
    infiziert_lebend_80plus ,
    ueberlapp_infiziertUNDgeimpft_80plus,
    testpositiv_lebend_80plus,
    neutod_geimpft_infiziert_80plus,
    neutod_60_80,
    neutod_ungeschuezt_60_80 ,
  ...
  res = list(neutod_ue60,  
             quantile,  
             quantile_schutzdet, 
             quantile_agedet,
             quantile_ageANDschutzdet, 
             tabelle)
  names(res) = c('neutod_ue60', 
                 'quantile',
                 "quantile_schutzdet",
                 "quantile_agedet",
                 "quantile_ageANDschutzdet", 
...
allscenarien_schutzdet$scenario = rep(
  c(
    "Scenario 1:\nIst-Stand", 
    "Scenario 2:\nImpfquote +10%",
    "Scenario 3:\n+Drittimpfung", 
...
p_scenarien = ggplot(
  ...
  aes(scenario, `50%`))  + 
  geom_point(size  = 44, alpha = 0.8, pch ="-") +
  ...
  ylab("Abschätzung zukünftige COVID-19 Verstorbene 60+\n(Median mit 95% Wahrscheinlichkeitsbereich)") + 
  xlab("")

p_scenarien

p_scenarien_schutzdet = ggplot(
  allscenarien_schutzdet, 
  aes(gruppe, 
      `50%`, 
      col = gruppe)) + 
...
allscenarien_agedet$scenario = rep(
  c(
    "Scenario 1:\nIst-Stand", 
    "Scenario 2:\nImpfquote +10%",
    "Scenario 3:\n+Drittimpfung", 
...
p_scenarien_ageANDschutzdet = ggplot(
  ...
  aes(gruppe, 
      `50%`, 
      col  = gruppe))  +
  geom_point(size  = 22, alpha = 0.8, pch ="-")  +
  scale_y_continuous(labels = label_comma(accuracy = 1),
                     breaks = pretty_breaks(15)) +
  facet_grid(.~scenario, 
             scales = "free", ) +
